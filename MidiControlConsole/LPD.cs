    ...
    public class LPD
	{
		public class MidiEvent
		{
			...
			public MidiEvent(int controller, float value)
			{
				...
				this.time = DateTime.Now.AddMilliseconds(8);
			}

			private float getNormalizedValue(float value)
			{
				// Max = 127
				float range = Math.Clamp(value, 5f, 120f) - 5f;
		...
		public void start()
		{
			while (true)
			{
				if (this.event_list.Count == 0)
				{
					wait_event.WaitOne();
				}

				var now = DateTime.Now;
				foreach (var key in this.event_list.Keys)
				{
					var event_item = this.event_list[key];
					if (now >= event_item.time)
					{
						SetVolumeMatchingSessionNames(this.lut[event_item.controller], event_item.value);
						while (!this.event_list.TryRemove(key, out _)) { }
					}
				}
			}
		}
		...
		private void midiIn_messageReceive(object sender, MidiInMessageEventArgs e)
		{
			...
			else
			{
				...
				this.event_list[controller] = pod_event;
				wait_event.Set();
			}
		}
